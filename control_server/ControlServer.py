...
def on_message(client, userdata, msg):
    ...
    # 자동 동작을 위한 쿼리
    query = {"topic":"home/livingroom_state/auto"}
    auto = db.states.find_one(query)
    if auto['state'] == 1:
        # 가스감지기 컨트롤
        ...
        if msg.topic == "home/livingroom_state/aircon":
            if value == 1:
                aircon.on()
            else:
                aircon.off()
        ...
        # 온도센서 컨트롤         
        elif msg.topic == "home/livingroom_state/heater":
            if value == 1:
                hitter.on()
            else:
                hitter.off()
        elif msg.topic == "home/livingroom_state/airdry":
            if value == 1:
                airdry.on()
            else:
                airdry.off()
